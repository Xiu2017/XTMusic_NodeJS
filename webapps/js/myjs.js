...
function search(next) {
	...
	if (!next) {
		...
		if (keyword != $("#search").val()) {
			...
			window.frames["audiolist"].$("td").removeAttr("value");
		}
	}
	...
	$.ajax({
		...
		success: function (result) {
			if (result != "-1") {
				...
				if (!songs) {
					window.frames["audiolist"].table.songs = res;
				} else {
					window.frames["audiolist"].table.songs = songs.concat(res);
				}
				if (res.length >= limit) {
					endOffset = false;
				}
...
//当输入框失焦时，显示提示文本
function inputBlur() {
	$("#search").attr("placeholder", "歌名/歌手/专辑");
}
...
function play_time() {
	...
	//刷新进度条
	var marginleft = 668 / audio.duration;
...
function showLrc() {
	...
	if (isShow == 1) {
		...
		$('#center_right').animate({ 'width': '999px' }, flashSpeed);
		...
		$('#center_right').animate({ 'width': '232px' }, flashSpeed);
